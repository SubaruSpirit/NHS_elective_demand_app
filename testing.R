...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Critical Resources", tabName = "critical_resources", icon = icon("plus")),
  ...
  dashboardBody(
    tabItems(
      ...
      # Critical Resources
      tabItem(tabName = "critical_resources",
              titlePanel("Critical Resources"),
              fluidRow(
                sidebarLayout(
                  sidebarPanel(
                    dateInput("critical_start", label = "Start Date",
                              format = "dd-mm-yyyy",
                              value = as.Date("01-05-2019", format="%d-%m-%Y")),
                    dateInput("critical_end", label = "End Date",
                              format = "dd-mm-yyyy",
                              value = as.Date("30-04-2020", format="%d-%m-%Y")),
                    selectInput("template_or_interactive", label = "Mode",
                                choices = c("Template", "Interactive"),
                                selected = "Template"),
                    numericInput("No_resources", "Number of critical resources",
                                 value = 1, min = 1, max = 100, step = 1),
                    width = 2
                  ),
                  mainPanel(
                    fluidRow(
                      column(4,
                             titlePanel("Critical Resources Template")
                      ),
                      column(4,
                             downloadButton("critical_download", "Download Template")
                      ),
                      column(4,
                             fileInput("critical_import",
                                       "Import", accept = c(".csv", ".tsv"))
                      )
                    ),
                    fluidRow(
                      reactableOutput("critical_template")),
                    width = 8
                  ))),
              fluidRow(
                column(2,
                       h4("Critical Resource")
                ),
                column(2,
                       h4("Period1")
                ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$capacity_start, {
    updateDateInput(
      session, "critical_start",
      
      value = input$capacity_start
    )
    updateDateInput(
      session, "critical_end",
      
      value = last(cdf1()[["Date"]])
    )
    disable("critical_start")
    disable("critical_end")
  })
  
  # table
  crdf1 = reactive({
    tibble(Critical_Resources=rep("",10), Start1="", End1="", Start2="", End2="",
           Start3="", End3="", Start4="", End4="", Start5="", End5="")
  })
  
  output$critical_template = renderReactable({
    if(input$template_or_interactive=="Template"){
      reactable(crdf1())
    } else {}
  })
  
  # download
  output$critical_download <- downloadHandler(
    filename = function(){"critical_resources.csv"}, 
    content = function(file){
      write.csv(crdf1(), file,row.names = FALSE)
    }
  )
  
  # import the data
  crdf2 <- reactive({
    req(input$critical_import)
    
    ext <- tools::file_ext(input$critical_import$name)
    switch(ext,
           csv = vroom::vroom(input$critical_import$datapath, delim = ",",
                              col_types = c(
                                Critical_Resources="c",
                                list(Start1=col_date("%d/%m/%Y"), End1=col_date("%d/%m/%Y"),
                                     Start2=col_date("%d/%m/%Y"), End2=col_date("%d/%m/%Y"),
                                     Start3=col_date("%d/%m/%Y"), End3=col_date("%d/%m/%Y"),
                                     Start4=col_date("%d/%m/%Y"), End4=col_date("%d/%m/%Y"),
                                     Start5=col_date("%d/%m/%Y"), End5=col_date("%d/%m/%Y"))
                              )
           ),
  ...
  observeEvent(input$critical_import,{
    output$critical_template <- renderReactable({
      reactable(crdf2())
    })
  })
  ...
  # interactive mode
  cr_names = reactive(paste0("cr", seq_len(input$No_resources)))
  period1 = reactive(paste0("a", seq_len(input$No_resources)))
  period2 = reactive(paste0("b", seq_len(input$No_resources)))
  period3 = reactive(paste0("c", seq_len(input$No_resources)))
  period4 = reactive(paste0("d", seq_len(input$No_resources)))
  period5 = reactive(paste0("e", seq_len(input$No_resources)))
  
  output$resources_names <- renderUI({
    map(cr_names(), ~ textInput(.x, label = .x, value = isolate(input[[.x]])))
  })
  
  output$period1 <- renderUI({
    map(period1(), ~ dateRangeInput(.x, label = .x, format = "dd-mm-yyyy",
                                    start = isolate(input[[.x]][1]),
                                    end = isolate(input[[.x]][2])))
  })
  
  output$period2 <- renderUI({
    map(period2(), ~ dateRangeInput(.x, label = .x, format = "dd-mm-yyyy",
                                    start = isolate(input[[.x]][1]),
                                    end = isolate(input[[.x]][2])))
  })
  
  output$period3 <- renderUI({
    map(period3(), ~ dateRangeInput(.x, label = .x, format = "dd-mm-yyyy",
                                    start = isolate(input[[.x]][1]),
                                    end = isolate(input[[.x]][2])))
  })
  
  output$period4 <- renderUI({
    map(period4(), ~ dateRangeInput(.x, label = .x, format = "dd-mm-yyyy",
                                    start = isolate(input[[.x]][1]),
                                    end = isolate(input[[.x]][2])))
  })
  
  output$period5 <- renderUI({
    map(period5(), ~ dateRangeInput(.x, label = .x, format = "dd-mm-yyyy",
                                    start = isolate(input[[.x]][1]),
                                    end = isolate(input[[.x]][2])))
  })
  ...
  colours2 = c("Core Capacity"="blue", "Ad-hoc"="green")
  ...
  output$plot2 = renderPlotly({
    ...
    if (input$capacity_unit=="Patients") {
      ...
      ggplot(cdf5(),aes(x=Date_Monday, y=`Core Capacity (Minutes)`))+
        geom_ribbon(aes(ymin=`Core Capacity (Minutes)`, ymax=`Total Capacity (Minutes)`,
                        fill="Ad-hoc"))+
        geom_ribbon(aes(ymin=0, ymax=`Core Capacity (Minutes)`, fill="Core Capacity"))+
        ylim(0,round(cdf5()[["total"]][1]/52)+2)+
        scale_x_date(date_breaks = "1 week", date_labels = "%d-%m-%Y")+
        theme_bw()+
        theme(axis.text.x = element_text(angle = 90, hjust = 1))+
