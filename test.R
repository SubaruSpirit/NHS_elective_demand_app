...
server <- function(input, output, session) {
  ...
  output$baseline_period = renderText({
    paste(
      as.numeric(
        difftime(input[[season_names()]][2], input[[season_names()]][1],
                 units = "weeks")
      ),"weeks")
  })
  ...
  output$plot1 = renderPlotly({   
    ...
    df7 = dplyr::filter( if(input$emergency=="No"){df6()}else{dfE3()},
                         between(Date, input[[season_names()]][1],
                                 input[[season_names()]][2])
    )
    ...
    # rectangle
    rect_range = tibble(xmin=as.Date(input[[season_names()]][1]),
                        xmax=as.Date(input[[season_names()]][2]),
